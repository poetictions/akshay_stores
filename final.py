...
products = {"Toiletries": [{"id":11, "name": "Toiletry Bag", "price": 1357, "discount": 0, "buying_price": 1200},
                           {"id":12, "name": "SPC Plastic Multipurpose Cosmetic Toiletries", "price": 500, "discount": 55, "buying_price": 450},
                           {"id":13, "name": "Tide Plus Double Power Detergent Washing Powder", "price": 930, "discount": 8, "buying_price": 890},
                           {"id":14, "name": "Dettol Liquid Disinfectant", "price": 193, "discount": 24, "buying_price": 100},
                           {"id":15, "name": "Colgate Strong Teeth", "price": 325, "discount": 0, "buying_price": 300},
                           {"id":16, "name": "Colgate Visible White O2 manual Toothbrush", "price": 250, "discount": 20, "buying_price": 200}],
            "Food": [{"id":21, "name": "Fresh Banana Yelakki", "price": 135, "discount": 32, "buying_price": 100},
                     {"id":22, "name": "Fresh Organic French Beans", "price": 51, "discount": 0, "buying_price": 44},
                     {"id":23, "name": "Purina Supercoat Adult Dry Dog Food", "price": 695, "discount": 70, "buying_price": 600},
                     {"id":24, "name": "Aveeno Skin Relief Moisturizing Lotion", "price": 695, "discount": 0, "buying_price": 590},
                     {"id":25, "name": "True Elements Chia Seeds", "price": 500, "discount": 33, "buying_price": 443},
                     {"id":26, "name": "Assorted pack of Ragi, Jowar & Moringa crispies", "price": 240, "discount": 0, "buying_price": 100}],
            "Electronics": [{"id":31, "name": "OnePlus Nord 2T 5G", "price": 32914, "discount": 12, "buying_price": 29000},
                            {"id":32, "name": "boAt Xtend/Xtend RTL Smartwatch", "price": 7990, "discount": 62, "buying_price": 6000},
...
class Customer:
    def __init__(self, id_number, name, room_number):
...
class ShoppingCart:
    ...
    def add_item(self):
        global cart
        cart = []
        a = input("ID of the product you want to add to your cart: ")
        b = input("Quantity")
        cart.append((a,b))
    
    def delete_item(self):
        a = input("ID of the product you want to delete from your cart: ")
        for i in cart:
            if a == (cart[i])[0]:
                cart.pop(i)
            else:
    ...
    def view_cart(self):
        for i in cart:
            print("ProductID: ", (cart[i])[0], "Name: ", "Quantity: ", (cart[i])[1] )
...
class Bill:
    ...
    def calculate_bill(self):
        ...
        for i in range(len(cart)):
            for j in range(a):
                for k in range(6):
                    if (cart[i])[0] == products[j][k]["id"]:
                        
                        actual_price = int(products[j][k]["price"])
                        total_actual_price = actual_price*cart[i][1]
                        
                        discounted_price = round(int(products[j][k]["price"]) - (int(products[j][k]["discount"]) * int(products[j][k]["price"]))/100, 2)
                        total_discounted_price = discounted_price*cart[i][1]
                                                
                        bill.append((cart[i][0], products[j][k]["name"], discounted_price,cart[i][1], total_discounted_price,actual_price, total_actual_price))
...
class AdminPrivileges:
    ...
    def view_all_products(self):        
        ...
        for i in range(6):
            ...
            if products["Toiletries"][i]["discount"] != "0":
                print("Discount: ", products["Toiletries"][i]["discount"],"%", "\nDiscounted Price: ","INR", round(int(products["Toiletries"][i]["price"]) - (int(products["Toiletries"][i]["discount"]) * int(products["Toiletries"][i]["price"]))/100, 2))
                profit = products["Toiletries"][i]["price"] - round(int(products["Toiletries"][i]["price"]) - (int(products["Toiletries"][i]["discount"]) * int(products["Toiletries"][i]["price"]))/100, 2)
                print("Buying_Price: ", products["Toiletries"][i]["buying_price"], "\nProfit: ",profit )
            else:
                pass
        ...
        for i in range(6):
            ...
            if products["Food"][i]["discount"] != "0":
                print("Discount: ", products["Food"][i]["discount"],"%", "\nDiscounted Price: ","INR", round(int(products["Food"][i]["price"]) - (int(products["Food"][i]["discount"]) * int(products["Food"][i]["price"]))/100, 2))
                profit = products["Food"][i]["price"] - round(int(products["Food"][i]["price"]) - (int(products["Food"][i]["discount"]) * int(products["Food"][i]["price"]))/100, 2)
                print("Buying_Price: ", products["Food"][i]["buying_price"], "\nProfit: ",profit )
            else:
                pass
        ...
        for i in range(6):
            ...
            if products["Electronics"][i]["discount"] != "0":
                print("Discount: ", products["Electronics"][i]["discount"], "%", "\nDiscounted Price: ","INR", round(int(products["Electronics"][i]["price"]) - (int(products["Electronics"][i]["discount"]) * int(products["Electronics"][i]["price"]))/100, 2))
                profit = products["Electronics"][i]["price"] - round(int(products["Electronics"][i]["price"]) - (int(products["Electronics"][i]["discount"]) * int(products["Electronics"][i]["price"]))/100, 2)
                print("Buying_Price: ", products["Electronics"][i]["buying_price"], "\nProfit: ",profit )            
            else:
                pass
...
class User:
    ...
    def category(self,category):        
        if category == "customer".lower():
            ...
            if a == "y".lower():
                b = input("Select a transaction option\n 1. SWD\n 2. UPI\n 3.Cash\n")
                if b == "1":
                    user = User()
                    user.student() 
        if category == "admin".lower():
            user = User()
            user.admin()
